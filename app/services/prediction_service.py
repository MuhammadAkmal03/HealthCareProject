...
class PredictionService:
    ...
    def __init__(self):
        ...
        if PredictionService._model_pipeline is None:
            ...
            try:
                ...
                logger.info(
                    "Successfully loaded prediction pipeline and symptom binarizer."
                )

            except FileNotFoundError as e:
                logger.error(
                    f"CRITICAL ERROR: Model artifact not found. The application will not be able to make predictions. {e}"
                )
    ...
    def predict(self, input_data: Dict) -> str:
        ...
        if self._model_pipeline is None or self._symptom_binarizer is None:
            ...
            raise RuntimeError(
                "Prediction models are not available. Please check server logs."
            )
        ...
        try:
            ...
            symptom_df = pd.DataFrame(
                symptom_encoded,
                columns=self._symptom_binarizer.classes_,
                index=df.index,
            )
